...
app.configure("development", function() {
  ...
  [["js", "updatedContent"], ["css", "updatedCss"]].forEach(function(group) {
    assetsSettings[group[0]].postManipulate["^"].push(function triggerUpdate(
      file,
      path,
      index,
      isLast,
...
app.configure(function() {
  ...
  app.use(
    express.logger({
      format:
        ":response-time ms - :date - :req[x-real-ip] - :method :url :user-agent / :referrer"
    })
  );
  ...
  if (siteConf.notifoAuth) {
    app.use(
      notifoMiddleware(siteConf.notifoAuth, {
        filter: function(req, res, callback) {
          callback(
            null,
            !req.xhr &&
              !(req.headers["x-real-ip"] || req.connection.remoteAddress).match(
                /192.168./
              )
          );
        },
        format: function(req, res, callback) {
          callback(null, {
            ...
            message:
              ":response-time ms - :date - :req[x-real-ip]/:remote-addr - :method :user-agent / :referrer"
          });
        }
      })
    );
